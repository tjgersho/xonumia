        ...
        userByToken(player.token).then(function (usr) {
            ...
            screenLevel(player.level, currentPlayer.userId).then(function (level) {
                ...
                db.map.findOne({ where: { id: currentPlayer.level } }).then(function (map) {
                    ...
                    currentPlayer.maptree = quadtree(map.minX, map.maxY, map.maxX - map.minX, map.maxY - map.minY);
...
function playerViewBoundBufferThreasholdReset(player) {
    player.viewBoundBufferThreashold = [player.y + player.screenCellsHeight / 2, player.x + player.screenCellsWidth / 2, player.y - player.screenCellsHeight / 2, player.x - player.screenCellsWidth / 2];
}

function checkMapBuffer(player) {

    if (player.viewBoundBufferThreashold[0] < player.y || player.viewBoundBufferThreashold[1] < player.x || player.viewBoundBufferThreashold[2] > player.y || player.viewBoundBufferThreashold[3] > player.x) {
...
function sendUpdates() {
    users.forEach(function (u) {
        ...
        var visibleXnum = xnum.filter(function (xx) {
            return xx.locX > u.x - 2 * u.screenCellsWidth && xx.locX < u.x + 2 * u.screenCellsWidth && xx.locY > u.y - 2 * u.screenCellsHeight && xx.locY < u.y + 2 * u.screenCellsHeight && parseInt(xx.level, 10) === parseInt(u.level, 10);
        });

        var visibleXbit = xbit.filter(function (xx) {
            return xx.locX > u.x - 2 * u.screenCellsWidth && xx.locX < u.x + 2 * u.screenCellsWidth && xx.locY > u.y - 2 * u.screenCellsHeight && xx.locY < u.y + 2 * u.screenCellsHeight && parseInt(xx.level, 10) === parseInt(u.level, 10);
        });

        var visiblePlayers = users.filter(function (g) {

            return g.x > u.x - u.screenCellsWidth / 2 - 5 && g.x < u.x + u.screenCellsWidth / 2 + 5 && g.y > u.y - u.screenCellsHeight / 2 - 5 && g.y < u.y + u.screenCellsHeight / 2 + 5 && parseInt(g.level, 10) === parseInt(u.level, 10);
        });
